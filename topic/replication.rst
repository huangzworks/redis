...
当主服务器持久化关闭时，复制功能的数据安全
-------------------------------

当配置Redis复制功能时，强烈建议打开主服务器的持久化功能。
否则的话，由于延迟等问题，部署的服务应该要避免自动拉起。

为了帮助理解主服务器关闭持久化时自动拉起的危害，参考一下下面会导致主从服务器数据全部丢失的例子：
...
如果Sentinel被用于实现Redis的高可用性，并且关闭主服务器上的持久化，同时开启自动拉起进程的话也很危险。
因为主服务器可能重启得非常快，以至于Sentinel没有检测到问题，然后就会发生上面的数据丢失的流程。
